...
def run(
    ...
    if (orders_df is not None):
        ...
        price_df = _create_price_df(
            price_df,
            pd.date_range(start_date, end_date, freq='T'),
            colname='close'
        )
        ...
        for index, order in orders_df.iterrows():
            ...
            if (order_type == 'SELL'):
                price *= (1 - impact)
                trades_df['Cash'].loc[index] += (price *
                                                 share_amount - commission)
                share_amount *= -1
            elif (order_type == 'BUY'):
                price *= (-1 * (1 + impact))
                trades_df['Cash'].loc[index] += (price *
            ...
            trades_df[order['symbol']].loc[index] = int(
                trades_df[order['symbol']].loc[index]) + int(share_amount)
...
def _create_price_df(
    price_df, dates, colname='adjusted_close'
):
